...
export const PerformanceTab = ({ selected, onSelect }: PerformanceTabProps) => {
  const [isOpen, setIsOpen] = useState(false);

  const hiddneItems = PERFORMANCE_TAP_ITEMS.slice(VISIBLE_MOBILE);
  const tabletHiddenItems = PERFORMANCE_TAP_ITEMS.slice(VISIBLE_TABLET);
  ...
  return (
    <div className="relative w-full max-w-[1400px] flex items-center justify-center">
      {PERFORMANCE_TAP_ITEMS.map((item, index) => (
        <button
          key={item.id}
